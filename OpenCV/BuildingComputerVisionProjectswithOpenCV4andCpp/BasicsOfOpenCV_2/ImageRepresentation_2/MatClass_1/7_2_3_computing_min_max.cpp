 ...
 *        a complex image. The problem with this exercise is that a signed value is satured once multiplication
...
void min_max_multichannel_extract_channel(){
    ...
    cv::Mat plane1{m1.rows, m1.cols, CV_8S, cv::Scalar(0)};
    cv::Mat plane2{m1.rows, m1.cols, CV_8S, cv::Scalar(0)};
...
void min_max_complex_magnitudes(){
    ...
    // cv::sqrt() We could calculate sqrt, but that's unnecessary calculation. Result is saturated if exceeds 
    // 127.
