"""
Me: So, there's a cool way we can combine select and insert together, to select from one table and insert into another. Consider we have a user_table and an address_table, where the first one has (id, name) and the second one has (id, email_address), where id is an increasing primary key. The point is that we want both of these two tables to put the email_address and the name in the same incrementing id, while at the same time, we supply the (name, email_address) pair. Wo, how do we do this with an actual querry?

A querry would look something like this:
INSERT INTO address VALUES (user.id, email_address) (SELECT user.id FROM user WHERE user.name == ?,  ?)
