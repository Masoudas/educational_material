...
*       with the value category it had when passed to the calling function. (me: so rvalue as rvalue and lvalue 
*       as lvalue.) 
...
*       -   If a call to wrapper() passes an rvalue std::string, then T is deduced to std::string (not 
*           std::string&, const std::string&, or std::string&&), and std::forward ensures that an rvalue 
*           reference is passed to foo. 
*       -   If a call to wrapper() passes a const lvalue std::string, then T is deduced to const std::string&, 
*           and std::forward ensures that a const lvalue reference is passed to foo.
*       -   If a call to wrapper() passes a non-const lvalue std::string, then T is deduced to std::string&, 
...
*      This overload makes it possible to forward a result of an expression (such as function call), which may 
*      be rvalue or lvalue, as the original value category of a forwarding reference argument.
*
*      For example, if a wrapper does not just forward its argument, but calls a member function on the argument,
*      and forwards its result (me: read to the definition of arg and don't panic!): 
...
* 
* Note: The following example is from the standard.
*/
