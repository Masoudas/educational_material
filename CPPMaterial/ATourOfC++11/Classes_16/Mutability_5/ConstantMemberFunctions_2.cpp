/**
 * To indicate to the compiler that the state of a class is not going to change inside the function.
 */
...
/**
 * Of course we can pass a constant reference to an object in a function as void f(constant Date&).
 ...
 * 
 * The problem here is that we need to check if cache is valid inside the method, and create the string. This
 * however can't be done!!! Such  problems  could  be  solved  through  brute  force  using  a  cast,  
 * for  example,  a const_cast(§11.5.2).   However,  there  are  also  reasonably  elegant  solutions  
 * that  do  not  involve messing  with type rules." See mutableModifier_3.cpp
 * 
