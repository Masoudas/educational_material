 ...
 * 
 * short is guaranteed at least 16 bits, long at least 32 bits. (char at least 8 bits).
 * The standard also indicates that int can have a size of at least 16 bits. Although on x86 x64 systems,
 * it would be at least 32.
 * 
 ...
 * 
 * Using an unsigned instead of an int to gain one more bit to represent positive integers is almost never a 
 * good idea. Attempts to ensure that some values are positive by declaring variables unsigned will typically 
 * be defeated by the implicit conversion rules.
 * 
 * Unlike plain chars, plain ints are always signed.  The signed int types are simply more explicit synonyms 
 * for their plain int counterparts, rather than different types.
 * 
 ...
 * size of an object. Me: Although this is compiler dependent. 
 * 
 */
#include <iostream>
using namespace std;
