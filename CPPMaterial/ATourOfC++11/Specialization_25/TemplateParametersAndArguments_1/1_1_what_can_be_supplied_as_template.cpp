...
* 2- Value parameters of BUILT-IN types such as ints, and pointers to functions (so not custom types).
...
* 
* Note of me: Recall that macros are namespace independent. Hence, even if a macro is defined in a namesoace, it
* will not be qualified with '::' to that namespace. This is the reason why templates may class with macros.
* 
...
* pass values of custom type 'S', we have to write something to the effect of 'template<S s>', but as we already 
* know:
