...
An axes of the current figure (Me: this is the way to supply axis).

subplots: bool, default False
Make separate subplots for each column.

sharex: bool, default True if ax is None else False
In case subplots=True, share x axis and set some x axis labels to invisible; defaults to True if ax is None otherwise False if 
an ax is passed in; Be aware, that passing in both an ax and sharex=True will alter all x axis labels for all axis in a figure.
...
data = [[1, 2], [3, 4], [5, 6]]
df = pd.DataFrame(data=data, columns=["column1", "column2"], copy=True)

# Plots column two versus column one
ax = df.plot(x="column2", y="column1", kind="bar", legend=False, 
    color=["red" if x > 3 else "blue" for x in df['column2']]) # The matplotlib axis returned from the plot.
plt.show()