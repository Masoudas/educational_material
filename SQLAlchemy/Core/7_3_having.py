"""
Recall that having statements were added on top of where statements, to ensure that we can use functions in where clauses. The idea here is that first we create a function with the column of interest, and then, we generate the column clause. Column clauses come usually with group by clauses.
