/**
 * In this example, we write the same stream, but as binary. This implies that we convert a floating point to 
 * the actual bits that form it.
 * 
 * Now, why would we want to do this? Well, for starters, this would allow us to stream any object we require 
 * directly into memory, not having to deal with individual elements. As such, serialization of any data form 
 * would be possible.
 * 
 * Notice that we did two important things here:
 * 	-	We opened the file in b format.
 * 	-	We used the fwrite function, which takes a pointer to void as argument. Also notice that the second 
 *       arg of this method is the element size, and third is the total number of elements.
 * 
 ...
 * 
 * Now, notice that using hexdump with the -C option, we can actually check out the content of the bits on 
 * the command line:
 * $ hexdump -C file
