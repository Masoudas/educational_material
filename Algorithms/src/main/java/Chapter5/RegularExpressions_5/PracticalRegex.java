 ...
 * Normally, we want to use the match case (alphabetically), so that exactly what we put in the search bar is searched for.
 * 
 ...
 * For example in abcabc abc, regex will find three matches. The pattern \babc\b however finds
 * only one. Note that order of course matters in the pattern. 
 * 
 * Note also that searching for special characters that are also part of regex special characters requires caution. 
 * We cannot directly search for .[{(|^\*+?&. For these we should use a raw string, or a scape backslash,
 * like \. (a dot) or \\. (a backslash, then any char).
 * 
 ...
 * 
 * \b - Word boundary (Start or end of a word. New line is also a end of word naturally.)
 ...
 * why would you?! The expression knows that these are literals (although this is not the case in VScode for example. It's best to
 * respect the escape dash). 
 * If dash put at the start or end, it just means dash, otherwise, a-z means range of from a to z. 
 * However, we can use special chars inside as well, like \d. 
 ...
 * of at least one char. (\w+|\d+) a a group of at least one word char or one digit. If I wanted to search for + in a group, I can write ([+]) for example. An empty paranthesis means any char. (^) means
 * beginning of string.
 * 
 * Quantifiers:
 * * Zero or more
